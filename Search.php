...
if ($srch==null)
	die("<h1><b>Input any search value</b></h1>");
mysql_select_db("AnimalPlanet");

$q="select * from Videos where Locate('$srch',VideoHeading)>0";
...
{
	$vid=$rec["VideoId"];
	$q="select count(*)as likecount from VideoLikes where VideoID=$vid";
	...
	print("</video><br>");
	$q1="select * from VideoLikes where LikeByUserId='$uid' and VideoId=$vid";
	//print($q1);
	$rs1=mysql_query($q1,$con);
	print("<div class=checkbox>");
	if($rec1=mysql_fetch_array($rs1,MYSQL_ASSOC))
	{
		print("<label><input type=checkbox class=chk value=$vid checked disabled /><span>Like($lc)</span></label>");
	}
	else
		print("<label><input type=checkbox class=chk value=$vid ><span>Like($lc)</span></label>");

	$q1="select * from favvideo where FavofUserId='$uid' and VideoId=$vid";
	//print($q1);
	$rs1=mysql_query($q1,$con);
	if($rec1=mysql_fetch_array($rs1,MYSQL_ASSOC))
		print("<label><input type=checkbox class=chk1 value=$vid checked disabled /><span>Favourite</span></label>");
	else
		print("<label><input type=checkbox class=chk1 value=$vid ><span>Favourite</span></label>");

	print("<button type=button class='bg-success but'>Info</button>");
	
	print("<div style='display:none'>");
	print("<h3>$vhead</h3>By: $UploadByUserID ($UploadDate)");
	print("</div>");
	print("</div>");
